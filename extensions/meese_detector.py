...
class MeeseDetector(commands.Cog):
    ...
    def has_meese(self, potentially_dirty_string, whitelist=[]):
        ...
        cleaned_string = self.replace_words(M_BLACKLIST, cleaned_string, "m",
                                            32)
        cleaned_string = self.replace_words(E_BLACKLIST, cleaned_string, "e",
                                            64)
            ...
            if not message.author.bot:
                ...
                if config.read(message.guild.id, "nomees"):
                    ...
                    if has_meese[0]:
                        ...
                        await message.reply(
                            util.get_message(
                                "meese.meese_detection",
                                nomees=nextcord.PartialEmoji.from_str(
                                    "nomees:936864150716575744"),
                            ))

                        await self.bot.logger.log(message.guild.id,
                                                  "meese detection",
                                                  message=message)
    ...
    async def report_meese_detection(self, interaction: nextcord.Interaction,
        ...
        await self.bot.logger.log(interaction.guild_id,
                                  "meese detection",
                                  message=message)
        ...
        await message.delete()
        await interaction.send("Successfully reported the message.",
                               ephemeral=True)
