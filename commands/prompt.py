...
class PromptCommand(commands.Cog):
  ...
  async def prompt_command(
    ...
    prompt: str=nextcord.SlashOption(
      description='The AI tries to "continue" this text. Try and keep under 3 sentences.'
    ),
    ...
    repetition_penalty: float=nextcord.SlashOption(
      description='Encourage the AI to talk about new things -- or at least to not repeat words it\'s used a lot',
      required=False,
      default=2.0,
      choices = {
        "Repetetive": 0.0,
    ...
    return_sequences: int=nextcord.SlashOption(
      description='How many times should you enslave the AI to complete your prompt?',
      required=False,
      default=1,
      min_value=1,
      max_value=3
    ...
    for output in raw_ai_output:
      ...
      if type(output) == str:
        ...
        complete_text = "**"+prompt+"**" + output['generated_text']

      # Cut it off at 2000 characters so Discord doesn't scream at us
      complete_text = complete_text[:1996] + (complete_text[1996:] and '...')
