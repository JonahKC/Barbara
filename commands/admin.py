...
class AdminCommand(commands.Cog):
  ...
  """
    ...
    @admin_command.subcommand(
        name="add", description="Add users or roles to the list of admins.")
    ...
    @admin_add_command.subcommand(
        name="user", description="Add a user to the list of admins.")
    async def admin_add_user_command(
        self,
        interaction: nextcord.Interaction,
        member: nextcord.Member = nextcord.SlashOption(
            description='The member to promote to admin.')):
        """
    ...
    @admin_add_command.subcommand(name="role",
                                  description="Add a role to admin roles.")
    async def admin_add_role_command(
        self,
        interaction: nextcord.Interaction,
        role: nextcord.Role = nextcord.SlashOption(
            description='The role to promote to admin.')):
        """
    ...
    @admin_remove_command.subcommand(
        name="user", description="Remove a user to the list of admins.")
    async def admin_remove_user_command(
        self,
        interaction: nextcord.Interaction,
        member: nextcord.Member = nextcord.SlashOption(
            description='The user to demote from admin')):
        """
    ...
    """
        ...
        await interaction.send(
            f"Demoted user {member.display_name} from admin.")

    @admin_remove_command.subcommand(
        name="role", description="Remove a role from the list of admin.")
    async def admin_remove_role_command(
        self,
        interaction: nextcord.Interaction,
        role: nextcord.Role = nextcord.SlashOption(
            description='The role to demote from admin.')):
        """
    ...
    @admin_command.subcommand(name="list",
    ...
    @admin_list_command.subcommand(name="roles",
                                   description="List manually set admin roles."
                                   )
    async def admin_list_roles_command(self,
                                       interaction: nextcord.Interaction):
        """
    List admin roles
    """
    ...
    @admin_list_command.subcommand(name="users",
                                   description="List manually set admin users."
                                   )
    async def admin_list_users_command(self,
                                       interaction: nextcord.Interaction):
        """
    ...
    """
        ...
        for i in users:
            ...
            users_temp.append(
                user.display_name.replace('_', '\_').replace('*', '\*'))

        # Send a message with the list
        await interaction.send('**ADMIN USERS:**\n' + '\n'.join(users_temp))

    @admin_list_command.subcommand(name="all",
    ...
    """
        ...
        for i in roles:
            ...
            if i == '@everyone':
                ...
                await interaction.send(
                    'Everyone on this server is an admin! The more the merrier!'
                )
