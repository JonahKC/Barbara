  ...
  """
    ...
    async def config_read_command(
        ...
        option: str = nextcord.SlashOption(
            ...
            default=False,
            description=
            'The config option to read. If left blank reads the whole config.')
    ):
        """
        ...
        try:
            ...
            if option:
                ...
                await interaction.send(
                    util.get_message('config.read_success',
                                     option=option,
                                     value=config.read(interaction.guild_id,
                                                       option)))
            else:

                # Otherwise send a formatted message with the entire config
                await interaction.send(
                    util.get_message('config.entire_config_read_success',
                                     guild_name=interaction.guild.name,
                                     config_json=json.dumps(config.load(
    ...
    @config_command.subcommand(name='set',
                               description='Set the value of a config option.')
    async def config_set_command(
        self,
        interaction: nextcord.Interaction,
        option: str = nextcord.SlashOption(
            description='The option to write to.'),
        value=nextcord.SlashOption(
            description='The value to write to the option.')):
        """
    ...
    """
        try:
            ...
            await interaction.send(
                util.get_message('config.write_success',
                                 option=option,
                                 value=value))
    ...
    async def config_reset_command(
        ...
        interaction: nextcord.Interaction,
        option: str = nextcord.SlashOption(
            description='The option to reset to default.')):
        """
    ...
    """
        try:
            ...
            # Send a success message
            await interaction.send(
                util.get_message('config.reset_success', option=option))
    ...
    @config_command.subcommand(name='append',
                               description='Add an item to a config list.')
    async def config_append_command(
        self,
        interaction: nextcord.Interaction,
        list: str = nextcord.SlashOption(
            description='The list to append a value to.'),
        value=nextcord.SlashOption(
    ...
    Append a value to a list in config. Get all configuration options by running /config read without a specific option
        ...
        try:
            ...
            # Send a success message
            await interaction.send(
                util.get_message('config.append_success',
                                 arr=list,
                                 value=value))
        ...
        # If there's an error appending to the array
            ...
            await interaction.send(err.__repr__()
                                   or 'Unknown `ConfigException` encountered')

    @config_command.subcommand(name='remove',
                               description='Remove an item from a config list.'
                               )
    async def config_remove_command(
        self,
        interaction: nextcord.Interaction,
        list: str = nextcord.SlashOption(
            description='The list to remove an element from.'),
        value=nextcord.SlashOption(
            description='The element to remove from the list.')):
        """
    ...
    """
        try:
            ...
            await interaction.send(
                util.get_message('config.remove_success',
                                 arr=list,
                                 value=value))
    ...
    async def config_restore_command(self, interaction: nextcord.Interaction):
        config.revert()
        await interaction.send(util.get_message("jcwyt.config.revert_success"),
                               ephemeral=True)
